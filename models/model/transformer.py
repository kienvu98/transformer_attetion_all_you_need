...
class Transformer(nn.Module):
    ...
    def __init__(self, src_pad_idx, trg_pad_idx, enc_voc_size, dec_voc_size, embedding,
    ...
    def make_trg_mask(self, trg):
        ...
        trg_pad_mask = (trg != self.trg_pad_idx).unsqueeze(1).unsqueeze(3)  # -> [batch_size, 1, 1, trg_len] tao padding mask
        
        # tao sub mask tam giacs duoi che tu dang truoc --> du doan tu dang truoc
        trg_len = trg.shape[1]
        trg_sub_mask = torch.tril(torch.ones(trg_len, trg_len)).type(torch.bool).to(self.device)      # -> [trg_len, trg_len]
        
        trg_mask = trg_pad_mask & trg_sub_mask.unsqueeze(0)    # -> [batch, 1, trg_len, trg_len]
        return trg_mask